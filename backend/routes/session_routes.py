...
def create_session_router(services) -> APIRouter:
    ...
    async def list_chat_sessions(http_request: Request):
        ...
        sessions = get_memory_store().list_sessions(user_id)
    ...
    async def get_chat_session_messages(http_request: Request):
        ...
        if cookie_session_id:
            ...
            if (
                get_memory_store().session_exists(cookie_session_id)
                and not get_memory_store().session_exists_for_user(cookie_session_id, user_id)
            ):
            ...
            session_id = str(uuid4())
        turns = get_memory_store().get_session_turns(session_id, user_id, limit=500)
    ...
    async def delete_chat_session(
        ...
        get_memory_store().delete_session(session_id, user_id)
    ...
    async def reset_chat_session(http_request: Request):
        ...
        get_memory_store().reset_session(session_id, user_id)
        await services.clear_checkpoint_thread(session_id)
        response = JSONResponse({"ok": True, "session_id": session_id})
        set_session_cookie(response, session_id)
        return response
