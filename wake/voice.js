/**
 * voice.js — Wake-word + utterance-capture frontend (extracted from Hearth).
 *
 * Requires:
 *   - A <button id="mic-btn"> in the page.
 *   - An <span id="voice-status"> (optional, for status text).
 *   - audio-processor.js served at /audio-processor.js (AudioWorklet).
 *   - A backend running wake.py at the same origin:
 *       WS  /ws/wake      — wake-word detection
 *       POST /transcribe  — Whisper transcription
 *
 * After a wake word is detected the script captures audio until the user
 * stops speaking, then POSTs the WAV to /transcribe and calls
 * window.onTranscription(text) with the result.  Replace that hook with
...
(() => {
  ...
  // ── State ────────────────────────────────────────────────────────────────────
  let voiceState = 'off'; // 'off' | 'sleeping' | 'recording' | 'transcribing'
  ...
  function setVoiceState(state) {
    ...
    if (voiceStatus) voiceStatus.textContent = state === 'off' ? '' : state;
  }
  ...
  async function transcribeAndSend(float32Audio) {
    ...
    try {
      ...
      if (text) {
        // Hook: replace this with your own handler.
        if (typeof window.onTranscription === 'function') {
          window.onTranscription(text);
        } else {
  ...
  async function startVoice() {
    if (location.protocol === 'file:') {
      alert('Open this page via HTTPS — voice features require a secure connection.');
      return;
    }

    if (!window.isSecureContext || !navigator.mediaDevices) {
      alert('Voice input requires a secure connection. Access this page via HTTPS, or from localhost.');
      return;
    }
    ...
    try {
      ...
      await _audioCtx.audioWorklet.addModule(new URL('/audio-processor.js', window.location.href));
